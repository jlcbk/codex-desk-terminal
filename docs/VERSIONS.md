...
| 字体栈（2026-09-11 定稿 v2） | GNU Unifont 17.0.05 全量主字体 | 主字体=Unifont 16px（8×16 等宽 ASCII + 16×16 全宽 CJK，57,054 字形全 BMP 公开指派码点零缺字；sha256 `b0913296…`，源 third_party/dl/unifont/，双许可 OFL 1.1/GPLv2+嵌入例外）；大字状态词=Montserrat 28；行高归一 18/基线 4（与 UI 布局零冲突）；fallback=替代符；转换器 scripts/gen_font_unifont.py（确定性+--check）；**wqy16 资产留仓可一键回切**（gen_font_wqy.py --check 实测可复现）；分区占用 app 3.2MB/4M（79%） | 用户真屏目检终选（等宽英文+笔画厚实+中文观感满意，2026-09-11）；wqy 轮三坑修复（进制/连续位流/LARGE）被继承 |
