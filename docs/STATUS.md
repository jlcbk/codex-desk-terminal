...
| ZC3 ZCode 数据侧填充（cwd/TodoWrite/审批内容/token累计） | A3（子代理） | doing | 真机 e2e+用户批准数据放行（2026-09-12 夜） | | | | 用户睡前授权按推荐自主决策；本轮三路并行之一 |
| ZC4 契约 v1.2+DETAILS 详情页（第 6 屏） | A4（子代理） | doing | A0 契约裁决（thread.model/tokens 可选字段+新页面导航） | | | | 同上；golden 不由子代理提交，A0 收编统一重生成 |
| ZC5 NOW 仪表盘化布局+USAGE/AGENTS 增强 | A5（子代理） | doing | 用户批准 NOW 页对齐效果图（2026-09-12 夜） | | | | 同上；不改页面导航枚举（ZC4 领地），shared 文件只许 Edit 不许整文件 Write |
