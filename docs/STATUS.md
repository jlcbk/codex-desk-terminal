...
| 真机用户追问增补：思考相位=THINKING | A0 | done | 上行+用户追问"思考中应显示 thinking？" | （本次提交） | ZCode 无实时 reasoning 信号（hooks 7 事件无思考事件、rollout reasoningText 事后才落盘）；A0 裁决相位映射：`UserPromptSubmit`/`PostToolUse`→合成 reasoning item（reducer 唯一 THINKING 路径，pending 优先级保 needs_you），`PreToolUse`→WORKING；needs_you 优先级经 `_effective_state` 不受影响；套件 193 passed（+1：提交→thinking→真实turn→working→done 全流+PostToolUse 相位） | bridge/sources/zcode.py、tests/bridge/test_zcode_mapper.py、docs/P3.6_DESKTOP_OBSERVATION.md §7.1 | 相位映射属"模型必然进入处理"的推断而非 reasoning 证据，已在代码注释+P3.6 §7.1 声明；若后续 ZCode 开放 reasoning hook 可换真证据 |
