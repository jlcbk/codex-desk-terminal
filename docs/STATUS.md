...
| ZCode 真机 e2e 首跑 | A0 | done（链路全通；needs_you 真机触发待日常使用积累） | ZC1/ZC2+用户重启 | 210b54e | 重启后 hooks 实证：dump 探针+spool 均收真实事件（SessionStart/UserPromptSubmit/PreToolUse，Z2 stdin schema 采到：session_id/toolName/riskLevel/turnId 双命名）；`bridge_serve_zcode` 真机跑通：设备 192.168.1.2 接入 clients=1、快照随本会话模型调用实时发布（seq 340→349 跟手）、观察器判定主会话 working+5 子代理 idle+终态门闸真数据吞 92 行；A0 两修复（provider lambda/1308 过期窗口抑制）后复测 190 passed | artifacts/zcode_serve/live_run.log、artifacts/zcode_hooks/dump.jsonl | 服务以后台 uv 进程常驻（exec 5abd1673）；主会话 project 名暂空（rollout 无 cwd 事实，spool stdin 有 cwd 可后续增补）；LONG-RUN 观察归 P6.2 |
