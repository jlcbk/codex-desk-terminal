#!/bin/sh
# build_power_tests.sh — P5.3 电源执行器主机端构建+测试（A3）
#
# 独立脚本：不追加/不改 build_firmware_io_tests.sh（P4.4/P4.5 可能正在写该
# 文件，避免并行冲突）；本脚本只编译 P5.3 域文件。
#
...
echo "[4/4] Gate 预检：电源纯逻辑层不得包含平台头"
...
if grep -rEn "$GATE_RE" \
    ...
    firmware/components/power/include/cdt_power_exec_pure.h; then
