/*
 * cdt_ui_details.c — DETAILS 会话详情页（ZC4，契约 v1.2 增补第 6 屏）
 *
 * 效果图第 6 屏：label: value 明细行展示"选中会话"（与 NOW 页同一选择规则：
 * selected_thread_id → 线程）的 PROJECT / MODEL / STATUS / DURATION / CONTEXT /
 * INPUT TOKENS / OUTPUT TOKENS / CACHED TOKENS。
 * 数值真源全部来自 ViewModel（presenter details builder 已按列预算截断；
 * 缺值统一 "--"，绝不编造）。v1.2 前的旧桥快照不含 model/tokens → 整列 "--"。
 * 布局构件与 USAGE 等页一致（cdt_ui_internal：标题栏/链路横幅/底栏）。
 */
#include <stdio.h>
...
#define DETAILS_ROW_H 26
#define DETAILS_ROW_Y0 64
#define DETAILS_ROW_COUNT 8
